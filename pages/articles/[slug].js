...
function Post({ post, asPath }) {
  ...
  return (
    ...
    >
      <BlogMedia alt={description} src={imageBuilder(mainImage).url()} />
      <Box mt={8}>
        <Flex direction="column" h="full" mb={12}>
          <Box>
            ...
            <BlogAuthor
              mt="8"
              name={author?.name}
              image={imageBuilder(author?.image).url()}
              role={author?.title}
              slug={author?.slug}
            />
          </Box>
          ...
          {products?.length > 0 && (
            <Accordion allowToggle defaultIndex={0} mt={12}>
              <AccordionItem>
                <h2>
                  <AccordionButton _expanded={{ color: "white", bg: "tomato" }}>
      ...
      <ShareIcons
        ...
        description={description}
        caption={description}
        media={imageBuilder(mainImage).url()}
        image={imageBuilder(mainImage).url()}
...
export const getServerSideProps = async ({ query, req: { url }, ...props }) => {
  ...
  return {
    props: {
      ...
      //       images: [
      //         {
      //           url: imageBuilder(mainImage).height(800).width(800).url(),
      //           width: 800,
      //           height: 800,
      //           alt: description,
      //         },
